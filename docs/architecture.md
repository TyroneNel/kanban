...
| CLI-backed task terminal | Claude Code, Codex, Gemini, OpenCode, Droid, Kiro, Cline CLI, and similar agents | task-scoped | PTY-backed process runtime | these agents are command-driven CLIs and already fit the terminal model well |
...
This is the path for Claude Code, Codex, Gemini, OpenCode, Droid, Kiro, Cline CLI, and any other command-driven agent.
...
| --- | --- | --- |
| task startup for Claude Code, Codex, Gemini, OpenCode, Droid, Kiro, or Cline CLI | the PTY runtime and agent launch path | accidentally adding special logic to the Cline path |
