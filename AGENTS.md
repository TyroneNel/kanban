...
- Kanban's native Cline agent is powered by the installed `@cline/core` and `@cline/llms` packages (the same lineage as the `cline` CLI) plus the local `src/cline-sdk/` boundary layer, so when Cline behavior is unclear, inspect those packages and `src/cline-sdk/` for the real implementation details. Do not remap `cline-pass` to `cline` at session start — the SDK registry knows ClinePass; only Cline account OAuth token-exchange uses the `cline` identity provider.
...
- Pi (`pi` from `@earendil-works/pi-coding-agent`) has no Claude-style Stop hook. Keep the interactive TUI; watch the per-task `--session-dir` JSONL in-process via `hooks pi-wrapper`. Do not switch task sessions to `--mode json`. Do not auto-restart on exit (session dir is durable). Do not add Pi-only recovery UX; completion and clean exit go to Review like other CLIs. Pass `agentSettings` as `--model` / `--thinking` / `--provider` verbatim.
- Cline CLI (`cline` 3.x, agent id `cline-cli`) differs from the legacy assumptions in the old `cline` terminal adapter: `--hooks-dir` only sets `CLINE_HOOKS_DIR`, which current CLI builds never read — hook scripts must live in `<cwd>/.cline/hooks/` named after hook events (`TaskComplete`, `PreToolUse`, ...). Never pass `--yolo` (it disables hook dispatch entirely) and never use `-s/--system` for the home-agent append prompt (it replaces the whole system prompt; use `.cline/rules/kanban-home-agent.md` instead). Auto-approve is ON by default, so non-autonomous launches need an explicit `--auto-approve false`. There is no `--continue`; resume is `--id <session-id>` only. A bare prompt runs one-shot and exits, so Kanban forces `--tui` to keep card terminals open.
